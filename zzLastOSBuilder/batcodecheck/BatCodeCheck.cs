...
namespace RobvanderWoude
{
	...
	{
		public static string progver = "0.41";


		#region Global Variables and Constants


		public const int maxvarlen = 32; // Some day, we may have to adjust the accepted variable name length if 32 characters is no longer enough ([\w\.-]{1,32})
		public const RegexOptions ignorecase = RegexOptions.IgnoreCase;
		public const string dynvarpattern = @"__APPDIR__|__CD__|CD|CMDCMDLINE|CMDEXTVESION|DATE|ERRORLEVEL|HIGHESTNUMANODENUMBER|RANDOM|TIME";
		public const string intcmdpattern = @"ASSOC|BREAK|CALL|CD|CHDIR|CLS|COLOR|COPY|DATE|DEL|DIR|DPATH|ECHO|ENDLOCAL|ERASE|EXIT|FOR|FTYPE|GOTO|IF|KEYS|MD|MKDIR|MKLINK|MOVE|PATH|PAUSE|POPD|PROMPT|PUSHD|RD|REM|REN|RENAME|RMDIR|SET|SETLOCAL|SHIFT|START|TIME|TITLE|TYPE|VER|VERIFY|VOL";
		public const string notx32pattern = @"BREAK|CTTY|FASTOPEN|FASTOPEN\.COM|FASTOPEN\.EXE|KEYS|SHARE|SHARE\.EXE";
		public const string notx64pattern = @"APPEND|APPEND\.COM|APPEND\.EXE|DEBUG|DEBUG\.COM|DEBUG\.EXE";
		public const string sysvarpattern = @"AllUsersProfile|AppData|CommonProgramFiles|CommonProgramFiles\(x86\)|CommonProgramW6432|ComputerName|ComSpec|HomeDrive|HomePath|LocalAppData|LogonServer|NUMBER_OF_PROCESSORS|OS|PATH|PATHEXT|PROCESSOR_ARCHITECTURE|PROCESSOR_IDENTIFIER|PROCESSOR_LEVEL|PROCESSOR_REVISION|ProgramData|ProgramFiles|ProgramFiles\(x86\)|ProgramW6432|PROMPT|SystemDrive|SystemRoot|TEMP|TMP|UserDomain|UserName|UserProfile|windir";
		...
		public static string checks = "ABELMSUV";
		...
		public static Dictionary<string, string> tests;
		...
		public static List<int> incorrectlinefeeds = new List<int>( );
		...
		static int Main( string[] args )
		{
			...
			#region Initialize List of Tests

			tests = new Dictionary<string, string>( );
			tests["A"] = "command line Arguments for batch commands";
			tests["B"] = "Best practice tips";
			tests["E"] = "Environment variables";
			tests["L"] = "Labels";
			tests["M"] = "common Mistakes";
			tests["S"] = "verbose Summary (variables, labels, subroutines)";
			tests["U"] = "Undefined environment variables";
			tests["V"] = "Vulnerabilities";

			#endregion Initialize List of Tests
				...
				if ( arg[0] == '/' )
				{
					switch ( arg.ToUpper( ).Substring( 0, Math.Min( 2, arg.Length ) ) )
					{
						...
						case "/C":
							...
							checks = arg.Substring( 3 ).ToUpper( );
							foreach ( char check in checks )
							{
								if ( !tests.Keys.Contains( check.ToString( ).ToUpper( ) ) )
								{
									return ShowHelp( "Invalid check \"{1}\" in command line argument \"{0}\"", arg, check.ToString( ) );
								}
							}
							checkall = false;
		...
		public static void CheckCommonMistakes( string line )
		{
			if ( checkall || checkcommerr )
			{
				...
				if ( regex.IsMatch( line ) )
				{
					message = "IF ERRORLEVEL 0 tests if the errorlevel is greater than or equal to 0, so for most native commands it ALWAYS returns true; use either IF NOT ERRORLEVEL 1 or IF %ErrorLevel% EQU 0 instead";
					allbadpr += string.Format( "{0}Line {1,5:0}:\t{2}", newline, linenum, message );
					badprmsg += string.Format( "{0}{1}.", newline, message );
					errinline = true;
					rc = linenum;
				...
				if ( regex.IsMatch( line ) )
				{
					message = "IF NOT ERRORLEVEL 0 tests if the errorlevel is less than 0, so for most native commands it always ALWAYS returns false; use either IF ERRORLEVEL 1 or IF %ErrorLevel% NEQ 0 instead";
					allbadpr += string.Format( "{0}Line {1,5:0}:\t{2}", newline, linenum, message );
					badprmsg += string.Format( "{0}{1}.", newline, message );
					errinline = true;
					rc = linenum;
				...
				if ( new Regex( @"(?:^|\b|\s|\()GOTO\s", ignorecase ).IsMatch( line ) )
				{
					...
					{
						string label = match.Groups[1].ToString( );
						if ( label.ToUpper( ) != "EOF" && !labels.Keys.Contains( label ) )
						{
				...
				// ELSE on new line
				#region ELSE on new line

				pattern = @"(^|\n)\s*ELSE";
				regex = new Regex( pattern, ignorecase );
				if ( regex.IsMatch( line ) )
				{
					// Missing error message and incorrect results reported by Bob Berry
					message = "ELSE should never be placed on a new line; placing ELSE on a new line will result in incorrect results without a warning or error message";
					alltips += string.Format( "{0}Line {1,5:0}:\t{2}", newline, linenum, message );
					tipsmsg += string.Format( "{0}{1}.", newline, message );
					errinline = true;
				}

				#endregion ELSE on new line
		...
		public static void CheckLinefeeds( )
		{
			string testlinefeeds = allsrc.Replace( "\r\n", "" );
			if ( testlinefeeds.Contains( "\r" ) || testlinefeeds.Contains( "\n" ) )
			{
				int linecount = 0;
				foreach ( string line in allsrc.Split( new string[] { "\r\n" }, StringSplitOptions.None ) )
				{
					...
					{
						incorrectlinefeeds.Add( linecount );
					}
				}
			}
		}
		...
		public static void CheckUndefinedVariables( string line )
		{
			// Ignore if test for undefined variables is excluded
			if ( checkundef )
			{
				string pattern;
				Regex regex;
		...
		public static void CollectEnvironmentVariables( )
		{
			...
			// Temporarily clone the keys into a new array because we are going to modify the dictionary while iterating through its keys
		...
		public static void CreateHTML( )
		{
			if ( log2html )
			{
				...
				htmlstream.WriteLine( "File date     :  {0}", File.GetLastWriteTime( source ).ToString( "yyyy-MM-dd HH:mm:ss" ) );
				htmlstream.WriteLine( "File encoding :  {0}", encoding );
				htmlstream.WriteLine( "Tests         :  {0}", checks );
				foreach ( char check in checks )
				{
					string test = check.ToString( ).ToUpper( );
					htmlstream.WriteLine( "                 {0} =  {1}", test, tests[test] );
				}
				if ( envvarerrors.Count > 0 )
				{
					htmlstream.Write( "Local environment variables are {1}", ( includelocalvars ? "included" : "ignored" ) );
				}
				htmlstream.WriteLine( "</span>\n" );
			}
		}
		...
		public static void DisplayMessagesSummary( )
		{
			...
			if ( encoding != "ASCII" )
			{
				allerrsum += string.Format( "{0}The batch file \"{1}\" is stored in {2} encoding, whereas it should be in ASCII", newline, source, encoding );
			}
			// Summary
			...
			else if ( lineswitherrors > 1 )
			{
				allerrsum += string.Format( "{0}{1,-3} lines generated warnings and should be examined", newline, lineswitherrors );
			}
			// Incorrect linefeeds
			if ( incorrectlinefeeds.Count == 1 )
			{
				allerrsum += String.Format( "{0}1   line is terminated with improper linefeeds (line {1})", newline, incorrectlinefeeds[0] );
			}
			if ( incorrectlinefeeds.Count > 1 )
			{
				allerrsum += String.Format( "{0}{1,-3} lines are terminated with improper linefeeds (lines {2})", newline, incorrectlinefeeds.Count, string.Join( ", ", incorrectlinefeeds.ToArray( ) ) );
			}
			// Unterminated %
			...
			else if ( unterminatedpercentsigns > 1 )
			{
			...
			else if ( unterminatedexclamations > 1 )
			{
			...
			else if ( unterminateddoublequotes > 1 )
			{
			...
			else if ( unterminatedsinglequotes > 1 )
			{
			...
			if ( !String.IsNullOrEmpty( allerrors ) )
			{
				...
				Console.WriteLine( "File encoding :  {0}", encoding );
				Console.WriteLine( "Tests         :  {0}", checks );
				foreach ( char check in checks )
				{
					string test = check.ToString( ).ToUpper( );
					Console.WriteLine( "                 {0} =  {1}", test, tests[test] );
				}
				if ( envvarerrors.Count > 0 )
				{
				...
				if ( logging )
				{
					...
					log.WriteLine( "File encoding :  {0}", encoding );
					log.WriteLine( "Tests         :  {0}", checks );
					foreach ( char check in checks )
					{
						string test = check.ToString( ).ToUpper( );
						log.WriteLine( "                 {0} =  {1}", test, tests[test] );
					}
					if ( envvarerrors.Count > 0 )
					{
		...
		{
			// Based on code by Jason Pierce (2Toad) on StackOverflow.com
		...
		public static void ParseLine( string line )
		{
			...
			CheckUndefinedVariables( nocomments );
		...
		public static void ParseLinePerCharacter2( string line )
		{
			...
			if ( regex.IsMatch( line ) )
			{
				...
				foreach ( Match match in matches )
				{
					...
					Console.Write( whitespace + intcmd );
					WriteSpanHTML( whitespace + intcmd );
					Console.ResetColor( );
		...
		public static int ShowHelp( params string[] errorMessage )
		{
			...
			/*
			BatCodeCheck,  Version 0.39 beta  (unlicensed)
			...
			*/
			...
			Console.Error.WriteLine( "BatCodeCheck,  Version {0} beta{1}", progver, ( licensed ? string.Empty : "  (unlicensed)" ) );
			...
			Console.Error.WriteLine( "         This program is based on regular expressions, it does not interpret" );

			Console.Error.WriteLine( "         the source code; hence false positives cannot be prevented entirely." );

			Console.Error.WriteLine( "         My motto when writing this program was \"I'd rather have 10 false" );

			Console.Error.WriteLine( "         positives than 1 missed error\"." );

			Console.Error.WriteLine( "         Use common sense when evaluating the warnings and error messages." );
